	...
	public List<byte[]> separate(byte[] arquivoPDF) throws PdfException {
		...
		try {
			//doc = PDDocument.load(arquivoPDF);
			 doc = Loader.loadPDF(new RandomAccessReadBuffer(arquivoPDF));
	...
	public byte[] join(List<byte[]> paginas) throws PdfException {
		...
		try {
			...
			for (byte[] item : paginas) {
				...
				PDFmerger.addSource(new RandomAccessReadBuffer(bis));
				bis.close();
			}
			...
			streamCache = IOUtils.createMemoryOnlyStreamCache();
