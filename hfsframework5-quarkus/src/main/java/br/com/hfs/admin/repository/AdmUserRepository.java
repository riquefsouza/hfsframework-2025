...
public class AdmUserRepository extends BaseRepository<AdmUser, Long> {
	...
	public List<?> findIPByOracle() {
		...
		Query query = getEntityManager().createNativeQuery("SELECT SYS_CONTEXT('USERENV', 'IP_ADDRESS', 15) FROM DUAL");
		return query.getResultList();
	}

	public List<?> findIPByPostgreSQL() {
		//TypedQuery<Object> query = em.createNamedQuery("AdmUser.findIPByPostgreSQL", Object.class);
		Query query = getEntityManager().createNativeQuery("SELECT substr(CAST(inet_client_addr() AS VARCHAR),1,strpos(CAST(inet_client_addr() AS VARCHAR),'/')-1)");
		return query.getResultList();
	}
	...
	public List<?> setLoginPostgreSQL(String login) {
		...
		Query query = getEntityManager().createNativeQuery("select set_config('myvars.usuario_login', ?1, false)");
	...
	public List<?> setIPPostgreSQL(String ip) {
		...
		Query query = getEntityManager().createNativeQuery("select set_config('myvars.usuario_ip', ?1, false)");
