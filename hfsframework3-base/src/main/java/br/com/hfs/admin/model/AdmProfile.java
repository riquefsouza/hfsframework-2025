...
@NamedQueries({
	...
	@NamedQuery(name = "AdmProfile.findProfilesByUser", 
	query = "SELECT distinct p FROM AdmProfile p, AdmUserProfile fp where p.id = fp.profileSeq AND fp.userSeq = ?1"),
	...
	@NamedQuery(name = "AdmProfile.findAdminMenuParentByProfile", 
	query="SELECT DISTINCT t FROM AdmMenu t WHERE t.id IN (SELECT m.idMenuParent FROM AdmProfile p INNER JOIN p.admPages f INNER JOIN f.admMenus m WHERE p = ?1 AND m.id <= 9) ORDER BY t.order, t.id"),
	@NamedQuery(name = "AdmProfile.findMenuParentByProfile", query="SELECT DISTINCT t FROM AdmMenu t WHERE t.id IN (SELECT m.idMenuParent FROM AdmProfile p INNER JOIN p.admPages f INNER JOIN f.admMenus m WHERE p = ?1 AND m.id > 9) ORDER BY t.order, t.id"),
	@NamedQuery(name = "AdmProfile.findAdminMenuByProfile", query="SELECT DISTINCT m FROM AdmProfile p INNER JOIN p.admPages f INNER JOIN f.admMenus m WHERE p = ?1 AND m.id <= 9 AND m.idMenuParent = ?2 ORDER BY m.order, m.id"),
	@NamedQuery(name = "AdmProfile.findMenuByProfile", query="SELECT DISTINCT m FROM AdmProfile p INNER JOIN p.admPages f INNER JOIN f.admMenus m WHERE p = ?1 AND m.id > 9 AND m.idMenuParent = ?2 ORDER BY m.order, m.id"),
	@NamedQuery(name = "AdmProfile.findAdminMenuParentByIdPerfis", query="SELECT DISTINCT t FROM AdmMenu t WHERE t.id IN (SELECT m.idMenuParent FROM AdmProfile p INNER JOIN p.admPages f INNER JOIN f.admMenus m WHERE p.id IN ?1 AND m.id <= 9) ORDER BY t.id, t.order"),
	@NamedQuery(name = "AdmProfile.findMenuParentByIdPerfis", query="SELECT DISTINCT t FROM AdmMenu t WHERE t.id IN (SELECT m.idMenuParent FROM AdmProfile p INNER JOIN p.admPages f INNER JOIN f.admMenus m WHERE p.id IN ?1 AND m.id > 9) ORDER BY t.order, t.id"),
	@NamedQuery(name = "AdmProfile.findAdminMenuByIdPerfis", query="SELECT DISTINCT m FROM AdmProfile p INNER JOIN p.admPages f INNER JOIN f.admMenus m WHERE p.id IN ?1 AND m.id <= 9 AND m.idMenuParent = ?2 ORDER BY m.id, m.order"),
	@NamedQuery(name = "AdmProfile.findMenuByIdPerfis", query="SELECT DISTINCT m FROM AdmProfile p INNER JOIN p.admPages f INNER JOIN f.admMenus m WHERE p.id IN ?1 AND m.id > 9 AND m.idMenuParent = ?2 ORDER BY m.id, m.order"),
