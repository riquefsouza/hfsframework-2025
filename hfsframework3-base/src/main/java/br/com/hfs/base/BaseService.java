...
public abstract class BaseService<T, I extends Serializable, C extends JpaRepository<T, I>>
	...
	@Override
	public List<T> findAllById(List<I> listIds) {
	...
	@Override
	public Long countMany(List<I> listIds) {
		TypedQuery<Long> query = em.createQuery("SELECT COUNT(x) FROM " + clazz.getName() + " x WHERE x.id IN ?1", Long.class);
		query.setParameter(1, listIds);
	...
	public List<T> insert(List<T> listEntity) throws TransactionException {
		try {
			return repository.saveAllAndFlush(listEntity);
		} catch (Exception e) {
			throw new TransactionException(log, ERROR_INSERT + e.getMessage(), e);
		}
	}
	...
	@Override
	public void deleteAllById(List<I> listIds) throws TransactionException {
		try {
			repository.deleteAllById(listIds);
		} catch (Exception e) {
			throw new TransactionException(log, ERROR_DELETE + e.getMessage(), e);
		}
	}
